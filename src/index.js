    ...
    const { currentPlayer, oppositePlayer, position, updateView } = gameContext; {
        ...
        taskQueue.push(onDone => {
            ...
            if (oppositeCard.length > 0) {
                oppositeCard.forEach(card => setTimeout(this.dealDamageToCreature(this.currentPower, card, gameContext, onDone)), 1000);
...
let bonus = 0;
Rogue.prototype.modifyDealedDamageToCreature = function(value, toCard, gameContext, continuation) {
    this.view.signalAbility(() => {
        for (let ability of Object.getOwnPropertyNames(Object.getPrototypeOf(toCard))) {
            if (ability == 'modifyDealedDamageToCreature' || ability == 'modifyDealedDamageToPlayer' || ability == 'modifyTakenDamage')
...
Rogue.prototype.modifyTakenDamage = function(value, fromCard, gameContext, continuation) {
...
Brewer.prototype.modifyDealedDamageToCreature = function(value, toCard, gameContext, continuation) {
    this.view.signalAbility(() => {
        let cards = gameContext.currentPlayer.table.concat(gameContext.oppositePlayer.table);
...
Nemo.prototype.doBeforeAttack = function(gameContext, continuation) {
    const { currentPlayer, oppositePlayer, position, updateView } = gameContext;
    this.view.signalAbility(() => {
        Object.setPrototypeOf(this, Object.getPrototypeOf(gameContext.oppositePlayer.table[position]));
        gameContext.updateView();
    });
